...
do while not eof()
  ...
  if found() 
    ...
    append blank
    replace code with uusi.c1, usedate with ctod('2014/01/01'), text with TRIM(uusi.c2), prim with .t., headline with .f., valid with .t., who with .f.
  endif
  select uusi
  skip
ENDDO
SELECT icd_10
SET FILTER TO NOT valid AND reldate=CTOD(SPACE(9))
